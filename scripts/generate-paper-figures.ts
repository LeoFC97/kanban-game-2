...
function writeCfdTikzFigure(csvRelative: string, outPath: string): void {
  const tex = `% Generated by scripts/generate-paper-figures.ts
\\begin{tikzpicture}
\\begin{axis}[
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  <rect width="100%" height="100%" fill="#ffffff"/>
  <text x="${m.l}" y="20" font-size="14" font-weight="bold">Tempo de ciclo médio por cenário</text>
  <line x1="${m.l}" y1="${m.t + innerH}" x2="${m.l + innerW}" y2="${m.t + innerH}" stroke="#333"/>
  <line x1="${m.l}" y1="${m.t}" x2="${m.l}" y2="${m.t + innerH}" stroke="#333"/>
  ${bars}
</svg>`;
...
const parametrosTex = `% Generated by scripts/generate-paper-figures.ts — do not edit by hand
\\begin{tabular}{@{}lr@{}}
  \\toprule
  Parâmetro & Valor \\\\
  \\midrule
...
const summaryTex = `% Generated by scripts/generate-paper-figures.ts
...
const tpFig = `% Generated by scripts/generate-paper-figures.ts
\\begin{tikzpicture}
\\begin{axis}[
  width=0.95\\linewidth,
  height=5.2cm,
...
console.log('Figures written under paper/figures and data under paper/data/.');
