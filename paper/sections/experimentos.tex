...
Para cada cenário, registramos: (i) série diária de contagens por coluna (base para visualizações estilo CFD); (ii) tempos de ciclo por cartão concluído (de \texttt{analise} a \texttt{deploy}), com média e mediana por cenário; (iii) throughput por sprint, atribuindo cada conclusão ao sprint do dia global correspondente. Os números deste artigo são produzidos por \texttt{runSimulation} no mesmo módulo \texttt{src/simulation/engine.ts} usado pela aplicação web (diferindo apenas no modo de acionamento: lote \emph{vs.} passo a passo). Todos os arquivos em \texttt{paper/data/} e \texttt{paper/figures/} podem ser regenerados a partir do mesmo commit do código, o que reduz risco de descompasso entre texto e implementação.
